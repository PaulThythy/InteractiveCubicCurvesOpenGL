...
void drawCatmullRom(Vector2* V, int N) {
    ...
    if(N >= 4){
        for(int k = 3; k < N; k++){
            ...
            Vector2 pk = V[k];
            catmullRom.setMatP(pkminus3, pkminus2, pkminus1, pk);

            double t = 0;
            while(t <= 1 - tSteps){
                catmullRom.setMatT(t);
                Matrix m1 = catmullRom.getMatT() * catmullRom.getMatM() * catmullRom.getMatP();
                Vector2 v(m1);

                t += tSteps;

                catmullRom.setMatT(t);
                Matrix m2 = catmullRom.getMatT() * catmullRom.getMatM() * catmullRom.getMatP();
                Vector2 v_next(m2);
